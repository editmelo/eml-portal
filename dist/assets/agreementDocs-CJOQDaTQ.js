import{e as b,p as y}from"./index-ze2urEtO.js";const v=1,w=30;function x(e){const a=new Date(e);return a.setFullYear(a.getFullYear()+v),a.toISOString()}function h(e){return Math.floor((new Date(e)-Date.now())/(1e3*60*60*24))}const E=b(y((e,a)=>({agreements:[],signAgreement:({docId:i,title:o,pdfPath:n,signerRole:r,signerId:t,signerName:s,signerEmail:g,signatureText:f})=>{const m=`${a().agreements.filter(l=>l.signerId===t&&l.docId===i).length+1}.0`,d=new Date().toISOString(),u=x(d),p={id:`agr_${Date.now()}`,docId:i,title:o,pdfPath:n,signerRole:r,signerId:t,signerName:s,signerEmail:g??"",signatureText:f,signedAt:d,expiresAt:u,ipNote:"Signed via EML Portal",version:m,status:"signed"};return e(l=>({agreements:[p,...l.agreements]})),p},getSignerAgreements:i=>a().agreements.filter(o=>o.signerId===i).sort((o,n)=>new Date(n.signedAt)-new Date(o.signedAt)),getActiveRecord:(i,o)=>{const n=a().agreements.filter(t=>t.signerId===i&&t.docId===o&&t.status==="signed");if(!n.length)return null;const r=n.sort((t,s)=>new Date(s.signedAt)-new Date(t.signedAt))[0];return new Date(r.expiresAt)>new Date?r:null},getLatestRecord:(i,o)=>{const n=a().agreements.filter(r=>r.signerId===i&&r.docId===o);return n.length?n.sort((r,t)=>new Date(t.signedAt)-new Date(r.signedAt))[0]:null},isActive:(i,o)=>a().agreements.some(n=>n.signerId===i&&n.docId===o&&n.status==="signed"&&new Date(n.expiresAt)>new Date),isExpired:(i,o)=>{const n=a().agreements.filter(t=>t.signerId===i&&t.docId===o&&t.status==="signed");if(!n.length)return!1;const r=n.sort((t,s)=>new Date(s.signedAt)-new Date(t.signedAt))[0];return new Date(r.expiresAt)<=new Date},isExpiringSoon:(i,o)=>{const n=a().agreements.filter(t=>t.signerId===i&&t.docId===o&&t.status==="signed").sort((t,s)=>new Date(s.signedAt)-new Date(t.signedAt))[0];if(!n)return!1;const r=h(n.expiresAt);return r>=0&&r<=w},daysRemaining:(i,o)=>{const n=a().agreements.filter(r=>r.signerId===i&&r.docId===o&&r.status==="signed").sort((r,t)=>new Date(t.signedAt)-new Date(r.signedAt))[0];return n?h(n.expiresAt):null}}),{name:"eml_agreements",partialize:e=>({agreements:e.agreements})})),S=`
...
`;function c(e){return new Date(e).toLocaleDateString("en-US",{month:"long",day:"numeric",year:"numeric"})}function A(e){return new Date(e).toLocaleString("en-US",{month:"long",day:"numeric",year:"numeric",hour:"2-digit",minute:"2-digit",timeZoneName:"short"})}function D(e){const{signerName:a,signedAt:i}=e,o=c(i);return`
...
<p class="intro">
  ...
  (the "Agency") and <strong>${a}</strong> (the "Subcontractor"), collectively "Parties",
  on this <strong>${o}</strong>.
</p>
...
`}function T(e){const{signerName:a,signedAt:i}=e;return`
<h1 class="doc-title">Non-Disclosure Agreement</h1>
<p class="intro">
  This Non-Disclosure Agreement ("Agreement") is entered into as of <strong>${c(i)}</strong>
...
`}function k(e){const a=e.docId==="nda"?T(e):D(e),i=A(e.signedAt),o=c(e.expiresAt);return`<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>${e.title} — ${e.signerName} — ${c(e.signedAt)}</title>
  <style>${S}</style>
</head>
<body class="has-bar">
  <!-- Print bar (hidden on print) -->
  <div class="print-bar no-print">
    <span>${e.title} &mdash; ${e.signerName}</span>
  ...
  <div class="page">
    ...
    <div class="sig-block">
      ...
      <div class="sig-row">
        ...
        <div class="sig-field" style="max-width:220px;">
          ...
          <div class="sig-value">
            <span class="sig-text">${c(e.signedAt)}</span>
          </div>
        </div>
      </div>
    ...
    <div class="cert-footer">
      ...
      <strong>${i}</strong>.<br/>
      Signer: <strong>${e.signerName}</strong>${e.signerEmail?" &lt;"+e.signerEmail+"&gt;":""} &nbsp;&bull;&nbsp;
      ${e.ipNote} &nbsp;&bull;&nbsp; Document v${e.version}<br/>
      This electronic signature is legally binding under the ESIGN Act (15 U.S.C. § 7001) and
      the Uniform Electronic Transactions Act (UETA).
    </div>
  </div>
</body>
</html>`}export{k as g,E as u};
