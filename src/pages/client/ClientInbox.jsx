...
export default function ClientInbox() {
  ...
  // Clients can only message designers assigned to their project
  const myProject      = projects.find((p) => p.id === user?.projectId)
  const designerIds    = myProject?.designerIds ?? []
  const allUsers       = [...MOCK_USERS, ...regUsers]
  const availableContacts = allUsers.filter(
    (u) => designerIds.includes(u.id) && u.id !== user?.id
  )
              ...
              <div className="flex flex-col items-center justify-center py-16 px-4 text-center">
                ...
                {availableContacts.length === 0 && (
                  <p className="text-[11px] text-slate-400 mt-2">
                    A designer will be assigned to your project soon.
                  </p>
                )}
              </div>
            ) : (
