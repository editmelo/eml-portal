/**
 * Client-facing document generator.
 *
 * Produces print-ready HTML for:
 *   generateProposalHtml(proposal)         — Project Proposal (preview, no signature)
 *   generateServiceAgreementHtml(sa)       — Service Agreement (signed doc)
 *   generateAddendumHtml(addendum)         — Addendum (signed doc)
 *
 * All docs open via:
...
function processingFee(total) {
  return Math.round(total * 0.03 * 100) / 100
}

// ─── Project Proposal ─────────────────────────────────────────────────────────

export function generateProposalHtml(proposal) {
  const sentDate    = proposal.sentAt ? fmt(proposal.sentAt) : fmt(new Date().toISOString())
  const validUntil  = proposal.validUntil ? fmt(proposal.validUntil) : ''
  const fee         = processingFee(proposal.totalAmount)
  const deposit     = proposal.depositAmount ?? Math.round(proposal.totalAmount * 0.5 * 100) / 100
  const balance     = proposal.totalAmount - deposit

  const scopeRows = (proposal.scopeItems ?? []).map((item) => `
    <tr><td>${item}</td></tr>
  `).join('')

  const versionNote = proposal.version > 1
    ? `<div style="text-align:center;font-size:9pt;color:#888;font-family:-apple-system,sans-serif;margin-bottom:8px;">Revision ${proposal.version - 1} — Updated ${sentDate}</div>`
    : ''
...
<head>
  ...
  <title>Project Proposal — ${proposal.projectName}</title>
...
<body class="has-bar">
  ...
  <div class="page">
    ...
    <div class="doc-subtitle">${proposal.projectName}</div>
    ...
    <div class="meta-row">
      ...
      <div class="meta-item">
        ...
        <div class="meta-value">${sentDate}</div>
      </div>
      <div class="meta-item">
      ...
      <div class="meta-item">
        ...
        <div class="meta-value">${proposal.serviceTitle}</div>
      </div>
    </div>
    ...
    <p>
      This proposal outlines the scope of work, deliverables, timeline, and investment for <strong>${proposal.projectName}</strong>. Our goal is to deliver a complete <strong>${proposal.serviceTitle}</strong> that meets your brand goals and exceeds expectations.
    </p>
    ...
    <p style="font-size:10pt;color:#555;font-family:-apple-system,sans-serif;">
      <strong>Estimated duration:</strong> ${proposal.timeline}
    </p>
    ...
    <table class="doc-table">
      ...
      <tbody>
        ${scopeRows || '<tr><td>To be defined</td></tr>'}
      </tbody>
    </table>
    ...
    <table class="doc-table">
      ...
      <tbody>
        <tr>
          <td>${proposal.serviceTitle} — ${proposal.packageTier ?? ''} Package</td>
          <td class="amount">${fmtCurrency(proposal.totalAmount)}</td>
        </tr>
        <tr>
          <td>Payment Processing Fee (3%)</td>
          <td class="amount">${fmtCurrency(fee)}</td>
        </tr>
        <tr class="total-row">
          <td>Total Investment</td>
          <td class="amount">${fmtCurrency(proposal.totalAmount + fee)}</td>
        </tr>
        <tr class="deposit-row">
          <td>50% Deposit (due upon signing — non-refundable)</td>
          <td class="amount">${fmtCurrency(deposit + processingFee(deposit))}</td>
        </tr>
        <tr>
          <td>Remaining Balance (due upon final delivery)</td>
          <td class="amount">${fmtCurrency(balance + processingFee(balance))}</td>
        </tr>
      </tbody>
    </table>
    ...
    <div class="cert-footer" style="margin-top:40px;">
      ...
      This proposal was prepared exclusively for <strong>${proposal.clientName}</strong> and is valid through <strong>${validUntil}</strong>. Scope and pricing are subject to change if the proposal expires.
    </div>
  </div>
</body>
</html>`
...
export function generateServiceAgreementHtml(sa) {
  const signedDate   = sa.signedAt ? fmt(sa.signedAt) : ''
  const effectiveDate = sa.signedAt ? fmt(sa.signedAt) : ''
  const fee          = processingFee(sa.totalAmount)
  const deposit      = sa.depositAmount ?? Math.round(sa.totalAmount * 0.5 * 100) / 100
  const balance      = sa.totalAmount - deposit

  const scopeRows = (sa.scopeItems ?? []).map((item) => `
    <tr><td>${item}</td></tr>
  `).join('')
...
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>Service Agreement — ${sa.projectName}</title>
  <style>${SHARED_STYLES}</style>
</head>
<body class="has-bar">
  <div class="print-bar no-print">
  ...
  <div class="page">
    ...
    <div class="doc-subtitle">${sa.projectName}</div>

    <div class="meta-row">
      <div class="meta-item">
        <div class="meta-label">Client</div>
        <div class="meta-value">${sa.clientName}</div>
      </div>
      <div class="meta-item">
      ...
      <div class="meta-item">
        ...
        <div class="meta-value">${effectiveDate}</div>
      </div>
    </div>

    <p>
      This Service Agreement (<strong>"Agreement"</strong>) is entered into as of <strong>${effectiveDate}</strong> by and between <strong>Edit Me Lo, LLC</strong>, a creative design studio (<strong>"Company"</strong>), and <strong>${sa.clientName}</strong> (<strong>"Client"</strong>).
    </p>
    ...
    <div class="sig-block">
      ...
      <div class="sig-row">
        <div class="sig-field">
          ...
          <div class="sig-value">
            <span class="sig-cursive">${sa.signatureText ?? ''}</span>
          </div>
        </div>
        <div class="sig-field">
          <p class="sig-label">Date Signed</p>
          <div class="sig-value">
            <span class="sig-text">${signedDate}</span>
          </div>
        </div>
      </div>
      <div class="sig-row">
        <div class="sig-field">
          ...
          <div class="sig-value">
            <span class="sig-text">${sa.clientName}</span>
          </div>
        </div>
        <div class="sig-field">
  ...
  <div class="page">
    <div class="attachment-header">
      ...
      <p>Attachment to Service Agreement for <strong>${sa.projectName}</strong></p>
    </div>

    <div class="meta-row">
      <div class="meta-item">
        <div class="meta-label">Service</div>
        <div class="meta-value">${sa.serviceTitle}</div>
      </div>
      <div class="meta-item">
        <div class="meta-label">Package</div>
        <div class="meta-value">${sa.packageTier ?? 'Custom'}</div>
      </div>
      <div class="meta-item">
        <div class="meta-label">Timeline</div>
        <div class="meta-value">${sa.timeline}</div>
      </div>
    </div>
    ...
    <table class="doc-table">
      ...
      <tbody>
        ${scopeRows || '<tr><td>As discussed and agreed</td></tr>'}
      </tbody>
    </table>

    ${sa.notes ? `<h2 class="section-head">Additional Notes</h2><p>${sa.notes}</p>` : ''}
    ...
    <table class="doc-table">
      ...
      <tbody>
        <tr>
          <td>${sa.serviceTitle} — ${sa.packageTier ?? ''} Package</td>
          <td class="amount">${fmtCurrency(sa.totalAmount)}</td>
        </tr>
        <tr>
          <td>Payment Processing Fee (3%)</td>
          <td class="amount">${fmtCurrency(fee)}</td>
        ...
        <tr class="total-row">
          <td>Total Investment</td>
          <td class="amount">${fmtCurrency(sa.totalAmount + fee)}</td>
        </tr>
        <tr class="deposit-row">
...
export function generateAddendumHtml(addendum) {
  const signedDate = addendum.signedAt ? fmt(addendum.signedAt) : ''
  const sentDate   = addendum.sentAt ? fmt(addendum.sentAt) : ''

  const itemRows = (addendum.additionalItems ?? []).map((item) => `
    <tr><td>${item}</td></tr>
  `).join('')
...
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>Addendum — ${addendum.title}</title>
  <style>${SHARED_STYLES}</style>
</head>
<body class="has-bar">
  <div class="print-bar no-print">
  ...
  <div class="page">
    ...
    <div class="doc-subtitle">${addendum.title}</div>

    <div class="meta-row">
      <div class="meta-item">
        <div class="meta-label">Project</div>
        <div class="meta-value">${addendum.projectName}</div>
      </div>
      <div class="meta-item">
        <div class="meta-label">Client</div>
        <div class="meta-value">${addendum.clientName}</div>
      </div>
      <div class="meta-item">
        <div class="meta-label">Date Issued</div>
        <div class="meta-value">${sentDate}</div>
    ...
    <p>
      This Addendum (<strong>"Addendum"</strong>) supplements and is incorporated into the existing Service Agreement between <strong>Edit Me Lo, LLC</strong> and <strong>${addendum.clientName}</strong>. All terms of the original Service Agreement remain in full force and effect.
    </p>
    ...
    <table class="doc-table">
      ...
      <tbody>
        <tr>
          ...
          <td class="amount">${fmtCurrency(addendum.additionalCost)}</td>
        </tr>
        <tr>
          <td>Payment Processing Fee (3%)</td>
          <td class="amount">${fmtCurrency(processingFee(addendum.additionalCost))}</td>
        </tr>
        <tr class="total-row">
          <td>Total Additional Cost</td>
          <td class="amount">${fmtCurrency(addendum.additionalCost + processingFee(addendum.additionalCost))}</td>
        </tr>
      </tbody>
    </table>` : `
    <h2 class="section-head">Additional Investment</h2>
    ...
    <div class="sig-block">
      ...
      <div class="sig-row">
        <div class="sig-field">
          <p class="sig-label">Client Signature</p>
          <div class="sig-value">
            <span class="sig-cursive">${addendum.signatureText ?? ''}</span>
          </div>
        </div>
        <div class="sig-field">
          <p class="sig-label">Date Signed</p>
      ...
      <div class="sig-row">
        <div class="sig-field">
          <p class="sig-label">Client Name</p>
          <div class="sig-value">
            <span class="sig-text">${addendum.clientName}</span>
          </div>
        </div>
        <div class="sig-field">
          <p class="sig-label">Service Provider</p>
    ...
    <div class="cert-footer">
      <strong>Electronic Signature Certification:</strong> This addendum was electronically signed by <strong>${addendum.clientName}</strong> on <strong>${signedDate}</strong> via the Edit Me Lo Client Portal. Document ID: <strong>${addendum.id}</strong>
    </div>
  </div>
</body>
</html>`
